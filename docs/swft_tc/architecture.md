...
inside that directory. Notebooks, scripts, tests and documentation live in the
repository's shared top-level folders, each under a matching `swft_tc/`
subfolder, so the model can be reviewed, moved or retired as one unit.
...
├── models/
│   └── swft_tc/
│       ├── __init__.py            # MODEL_ROOT / REPO_ROOT: the single path anchor
│       ├── config/
│       │   ├── config.yaml
│       │   ├── config.example.yaml
│       │   └── group_config.csv
│       ├── data/
│       │   ├── sample_input.csv
│       │   ├── sample_expected_group15.csv
│       │   └── reference/
│       │       ├── iso3166.csv    # approved source in a real environment
│       │       ├── town_country_reference.csv   # external dependency; git-ignored
│       │       └── PROVENANCE.md
│       ├── outputs/
│       │   ├── errors/            # processing_errors.csv
│       │   └── reports/           # aggregate-only artifacts (tracked)
│       ├── prompts/
...
├── notebooks/
│   └── swft_tc/
│       ├── 01_phase1_address_extraction_DRY_RUN.ipynb
│       └── 01_phase1_address_extraction_ACTUAL_RUN.ipynb
├── scripts/
│   └── swft_tc/
│       ├── run_batch.py           # batch entry point; no business logic
│       └── build_geonames_town_country_reference.py
├── tests/
│   └── swft_tc/
│       ├── conftest.py
│       ├── fixtures/
...
**Imports.** There is one convention, used identically by the notebooks, the
batch script and the tests:

```python
from models.swft_tc.src.pipeline import run_pipeline
```

The repository root is the only entry on the import path. `pytest.ini` supplies
it for the test suite; the batch script and each notebook add it once from a
location derived from their own file or from the repository markers. No module
under `models/swft_tc/src/` manipulates `sys.path`, and nothing resolves a path
