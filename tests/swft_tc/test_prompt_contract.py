...
from models.swft_tc.src.schemas import (
    NO_COUNTRY,
    NO_TOWN,
...
class TestAeronauticaContract:
    ...
    def test_expected_response_shape_for_aeronautica(self, iso_provider):
        from models.swft_tc.src.scoring import verify_extraction
    ...
    def test_a_rona_claim_is_stripped_of_its_explicitness(self, iso_provider):
        from models.swft_tc.src.scoring import evaluate
...
def _scoring_config():
    ...
    return load_config(
        MODEL_ROOT / "config" / "config.yaml", base_dir=MODEL_ROOT
    ).scoring
