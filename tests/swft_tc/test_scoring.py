...
from models.swft_tc.src.scoring import (
...
class TestConfigValidation:
    def test_missing_required_scenario_is_rejected(self, model_root, tmp_path):
        import yaml

        from models.swft_tc.src.settings import load_config

        source = yaml.safe_load((model_root / "config" / "config.yaml").read_text())
        ...
        with pytest.raises(Exception, match="both_explicit"):
            load_config(broken, base_dir=model_root)

    def test_weight_outside_unit_interval_is_rejected(self, model_root, tmp_path):
        import yaml
