...
| Referenced by | `models/swft_tc/config/config.yaml` → `reference_data.iso3166_path` |
| Context version | `reference_data.reference_context_version` (participates in the cache key) |

### Why this file exists as a fallback

`config.example.yaml` points at `data/reference/iso3166.csv` (relative to the
model root), but no such file
...
into `models/swft_tc/outputs/run_metrics.json` for every run, so no run can quietly claim
...
| Referenced by | `models/swft_tc/config/config.yaml` → `reference_data.town_country_path` |
| Git status | ignored via `models/swft_tc/data/reference/*.csv` |
...
Unit tests use `tests/swft_tc/fixtures/town_country_reference_test.csv` — a 17-row schema sample — and never
...
licensed data. `SwiftRefProvider` in `models/swft_tc/src/reference_data.py` is a
