...
#: Root of the model package — `models/swft_tc/` — and the enterprise repository
#: root. Both are derived once, in this package's ``__init__``, from the module
#: location rather than the working directory, so a run launched from the
#: repository root, a notebook directory or a scheduler resolves paths
#: identically. They are re-exported here because ``settings`` is where every
#: relative configured path is turned into an absolute one; nothing anywhere in
#: the codebase consults ``Path.cwd()``.
from .. import MODEL_ROOT, REPO_ROOT  # noqa: E402
...
class ProcessingConfig(_Base):
    ...
    errors_path: str = "outputs/errors/processing_errors.csv"
...
def load_config(
    ...
    *,
    ...
    ``config/config.example.yaml`` — each resolved against ``base_dir``, which
    defaults to :data:`MODEL_ROOT` rather than the working directory.
    ...
    editing the file.
    """
    root = Path(base_dir) if base_dir is not None else MODEL_ROOT
